  ...
  <div class="flex flex-col gap-2 w-full "> 
    ...
    @foreach($medias as $media)
      <div class="flex gap-2">
        ...
        <div class="flex">
          <img src="{{ $media->cover_image_path }}" 
            alt="{{ $media->title_english }} cover image"
            class="w-20 h-28">
        </div>
        <div class="flex flex-col">
          @if($media->title_english != NULL)
            <a href="/{{ $type }}s/{{$media->id}}" class="text-xl text-(--logo-blue) font-bold">{{ $media->title_english }}</a>
          @else
            <a href="/{{ $type }}s/{{$media->id}}" class="text-xl text-(--logo-blue) font-bold">{{ $media->title_romaji }}</a>
          @endif
          <p class="text-[#505050]">Score: {{ $media->average_score }}/100</p>
          <p class="text-[#505050]">Status: {{ $media->status }}</p>
